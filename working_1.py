...
class MyModeler:
    def __init__(self, data, modelfile, *, window=1000, horizon=300, resample_rate='1s', n_epochs=1, modeltype=NHiTSModel, **modelparams):
        self.data = data
    ...
    def init_model(self):
        try:
            ...
            #self.model = torch.load(self.model_file)  # Use PyTorch's torch.load for loading model
            self.model = torch.load(self.model_file, weights_only=False)
    ...
    def save_model_weights(self):
        ...
        torch.save(self.model, self.model_file)  # Save model using PyTorch
...
class LiveTrading:
    def __init__(self, modeler, data_fetch_func, update_interval=60, tolerance=0.0007, tcosts=0.000025):
...
mymod5 = MyModeler(qqqdf, modelfile='qqqmod_NHiTS_11.pth', window=1000, horizon=300, modeltype=NHiTSModel)
mymod5.fit_span(end_day='2024-10-01')
...
live_trader = LiveTrading(mymod5, fetch_live_btc_data, update_interval=60)
live_trader.run()
